...
2.  **Services:** The route calls a service method, which contains the business logic. It validates input, orchestrates operations, and decides what to do (e.g., create a `Bookmark` object).
3.  **Repository:** The service uses the repository to interact with the data store. This layer abstracts away the details of how data is saved, retrieved, or deleted.
4.  **Models:** Plain Python classes (`Bookmark`, `Tag`, `Collection`) represent the data structures used throughout the application.
